...
export default function AdminEditBlogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) return null;
  return <EditBlogEditor blogId={blogId} adminId={user._id} />;
}

function EditBlogEditor({ blogId, adminId }: { blogId: string; adminId: string }) {
  ...
  useEffect(() => {
    adminApi.getAdminBlogForEdit(blogId, adminId)
      .then((res) => {
        const b = res.data;
        setBlog(b);
        setTitle(b.title ?? "");
        ...
        if ((BLOG_CATEGORIES as readonly string[]).includes(storedCat)) {
  ...
  const handleSaveDraft = async () => {
    ...
    setSaving(true);
    try {
      await adminApi.saveAdminBlogAsDraft(adminId, { id: blogId, ...payload() });
  ...
  const handlePublish = async () => {
    ...
    setPublishing(true);
    try {
      await adminApi.publishAdminBlogEdit(blogId, adminId, payload());
  ...
  const handleDiscard = async () => {
    setDiscarding(true);
    try {
      await adminApi.discardAdminBlog(blogId, adminId);
  ...
  }, [adminId, blogId, title, slug, category, otherCategory, tags, content]);

  const { autoSaveStatus } = useAutoSave(autoSaveFn, [title, slug, category, tags, content], {
    canSave: !!title.trim() && !loadingBlog,
  });

  if (loadingBlog) return <EditorSkeleton />;
      ...
      <Dialog.Root open={showPreview} onOpenChange={setShowPreview}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-4xl max-h-[90vh] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-border bg-card p-8 shadow-xl">
            ...
            <p className="text-xs text-muted-foreground mb-6">By {authorName} · Status: {blog?.status}</p>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-border bg-card p-6 shadow-xl">
            ...
            <Dialog.Description className="mt-2 text-sm text-muted-foreground">
              This will move &ldquo;{title}&rdquo; to the discard list.
            </Dialog.Description>
            <div className="mt-5 flex justify-end gap-2">
              <Dialog.Close asChild><Button variant="outline" size="sm">Cancel</Button></Dialog.Close>
              <Button variant="destructive" size="sm" disabled={discarding} onClick={handleDiscard} className="gap-1.5">
                {discarding ? <Loader2 className="size-3.5 animate-spin" /> : <FileMinus className="size-3.5" />}Discard
              </Button>
            </div>
          </Dialog.Content>
        </Dialog.Portal>
      ...
      <main className="mx-auto max-w-3xl px-6 py-10">
        <div className="mb-6 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-serif text-xl font-semibold truncate">{blog?.title ?? "Edit blog"}</h1>
            <p className="text-xs text-muted-foreground mt-0.5">
              By <span className="font-medium text-foreground">{authorName}</span>
              {blog?.status && <> · <Badge variant="outline" className="text-xs ml-1">{blog.status}</Badge></>}
            </p>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-3 pt-2 border-t border-border">
            ...
            <Button variant="ghost" size="sm" className="gap-1.5 text-destructive hover:text-destructive" onClick={() => setDiscardOpen(true)}>
...
function AutoSaveIndicator({ status }: { status: string }) {
  ...
  if (status === "saving") return <span className="flex items-center gap-1 text-xs text-muted-foreground"><Loader2 className="size-3 animate-spin" />Saving…</span>;
  if (status === "saved") return <span className="flex items-center gap-1 text-xs text-muted-foreground"><Cloud className="size-3" />Saved</span>;
  return <span className="flex items-center gap-1 text-xs text-destructive"><CloudOff className="size-3" />Auto-save failed</span>;
}
