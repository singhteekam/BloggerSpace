...
function SitemapCard() {
  ...
  const checkSitemap = async () => {
    ...
    try {
      ...
      toast.success(res.data?.message ?? "Sitemap OK!");
    } catch (err) {
      toast.error(isAxiosError(err) ? (err.response?.data?.error ?? "Failed to reach sitemap endpoint.") : "Error.");
    } finally {
      setUpdating(false);
    }
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Auto-generated by Next.js at{" "}
            <code className="text-xs bg-muted px-1 py-0.5 rounded">/sitemap.xml</code>{" "}
            · refreshes every hour. Click to verify URL count.
          </p>
        </div>
      </div>
      <Button variant="outline" size="sm" className="gap-1.5 shrink-0" disabled={updating} onClick={checkSitemap}>
