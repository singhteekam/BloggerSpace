...
// Fetch blogs by page (server handles pagination)
async function fetchBlogs({ queryKey }) {
  const [_key, { page, limit }] = queryKey;
  const { data } = await axios.get("/api/blogs/fetchallblogs", {
    params: { page, limit },
  });
  return data; // contains blogs, currentPage, totalPages
}
...
const AllBlogs = () => {
  ...
  // ✅ Sync URL params
  useEffect(() => {
    setSearchParams({ page, search: searchTerm, filterType, filterValue });
        ...
        {filteredBlogs.length === 0 ? (
          ...
          <Row className="m-3">
            {filteredBlogs.map((blog) => (
              <Col md={4} key={blog._id || blog.blogId}>
                      ...
                      <Card className="blogcard bgcolor-mint">
                        ...
                        <Card.Body>
                          ...
                          <p>
                            ...
                            <i className="text-muted">
                              Last Updated:{" "}
                              {new Date(blog.lastUpdatedAt).toLocaleString()}
                            </i>
                            <br />
